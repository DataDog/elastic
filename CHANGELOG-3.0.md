...
It also helps to prevent [subtle issues](https://github.com/DataDog/elastic/issues/115#issuecomment-130753046).
...
Errors and all its details are now returned in [`Error`](https://github.com/DataDog/elastic/blob/release-branch.v3/errors.go#L59).
...
To check for HTTP Status 404 (with non-HEAD requests), e.g. when trying to get or delete a missing document, you can use the [`IsNotFound`](https://github.com/DataDog/elastic/blob/release-branch.v3/errors.go#L84) helper (see below).
...
To check for HTTP Status 408, we introduced the [`IsTimeout`](https://github.com/DataDog/elastic/blob/release-branch.v3/errors.go#L101) helper.
...
These errors are now captured in an object of type [`ErrorDetails`](https://github.com/DataDog/elastic/blob/release-branch.v3/errors.go#L59) which is used in [`BulkResponseItem`](https://github.com/DataDog/elastic/blob/release-branch.v3/bulk.go#L206).
...
Services generally return a typed response from a `Do` func. Those structs are exported so that they can be passed around in your own application. In Elastic 3.0 however, we changed that (most) sub-structs are now unexported, meaning: You can only pass around the whole response, not sub-structures of it. This makes it easier for restructuring responses according to the Elasticsearch API. See [`ClusterStateResponse`](https://github.com/DataDog/elastic/blob/release-branch.v3/cluster_state.go#L182) as an example.
